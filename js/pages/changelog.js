...
const Changelog = (() => {
  ...
  const ENTRIES = [
    ...
    },
    {
      ...
      time: '16:30',
      tags: ['privacy'],
      title: 'CGU / CGV mises à jour (abonnements)',
      titleEn: 'Terms updated (subscriptions)',
      items: [
        { type: 'privacy', text: 'Les Conditions Générales (CGU/CGV) ont été mises à jour : offres actuelles (Trader / Funded / Elite), paiement par abonnement Stripe, et renouvellement & résiliation en 1 clic.', textEn: 'Terms updated: current plans (Trader / Funded / Elite), Stripe subscription billing, and renewal & 1-click cancellation.' },
      ],
    },
    {
      version: '0.9.253',
    ...
    },
    {
      ...
      items: [
        ...
        { type: 'feature', text: 'Bandeau de confiance sous les 3 cartes : "Annulation 1-clic · 7 jours d\'essai gratuit · Export complet RGPD · Aucune CB requise".', textEn: 'Trust banner under the 3 cards: "1-click cancellation · 7-day free trial · Full GDPR export · No credit card needed".' },
    ...
    },
    {
      ...
      time: '00:30',
      tags: ['ui', 'auth'],
      title: 'Bouton 👁 pour voir le mot de passe (login + signup)',
      titleEn: 'Show password button (login + signup)',
      items: [
        { type: 'ui', text: 'Ajout d\'un bouton 👁 à droite des 3 champs mot de passe (connexion + inscription mot de passe + confirmation). Clic = toggle entre `type="password"` (caché) et `type="text"` (visible). SVG inline (CSP-friendly), event delegation sur `document.body` pour robustesse. Bouton `tabindex="-1"` (pas dans le tab order pour ne pas gêner le flow clavier).', textEn: 'Added a 👁 button next to all 3 password fields (login + signup password + confirm). Click = toggle between `type="password"` (hidden) and `type="text"` (visible). Inline SVG (CSP-friendly), event delegation on `document.body` for robustness. Button `tabindex="-1"` (out of tab order to not disrupt keyboard flow).' },
      ...
      user: {
        ...
        items: [
          { type: 'ui', text: 'Sur la page connexion et inscription, clique sur l\'icône 👁 à droite du champ mot de passe pour l\'afficher en clair. Re-clique pour le masquer. Utile pour vérifier que tu n\'as pas fait de typo.' },
        ],
      },
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { type: 'admin', text: 'UI admin enrichie : (1) onglet Outils → nouveau bloc "Forcer la vérification email" avec bouton bulk "✉ Marquer tous les emails comme vérifiés" (confirmation obligatoire) ; (2) onglet Utilisateurs → chaque ligne user a maintenant un bouton "✉ Vérifier email" pour action ciblée (style bleu pour distinguer des autres actions).', textEn: 'Enriched admin UI: (1) Tools tab → new "Force email verification" block with bulk button (mandatory confirm); (2) Users tab → each user row has a new "✉ Verify email" button for targeted action.' },
